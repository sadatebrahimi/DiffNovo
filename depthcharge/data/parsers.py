...
class BaseParser(ABC):
    ...
    def read(self):
        ...
        if self.spectra:
            self.scans = [key for key in self.spectra.keys()]
            self.mz_arrays =[v['mz_arrays'] for k,v in self.spectra.items()]
            self.intensity_arrays =[v['intensity_arrays'] for k,v in self.spectra.items()]
...
class MgfParser(BaseParser):
    ...
    def parse_spectrum(self, spectrum):
        ...
        if self.ms_level > 1:
            ...
            if scans in self.spectra:
                #only need to update m/z and intensity lists
                #collect all spectra assigned to a precursor ion
                self.spectra[scans]['mz_arrays'] = np.concatenate( (self.spectra[scans]['mz_arrays'], mz_list))
                self.spectra[scans]['intensity_arrays'] =np.concatenate((self.spectra[scans]['intensity_arrays'], intensity_list))
            elif self.valid_charge is None or precursor_charge in self.valid_charge:
                    self.spectra[scans] = dict({'mz_arrays':[], 'intensity_arrays':[]})
                    if self.annotations is not None:
                        self.annotations.append(spectrum["params"].get("seq"))
                    self.rt_array.append(retention_time)
                    self.precursor_mz.append(float(spectrum["params"]["pepmass"][0]))
                    self.precursor_charge.append(precursor_charge)
                    self.spectra[scans]['mz_arrays'] = mz_list
                    self.spectra[scans]['intensity_arrays'] = intensity_list
                    self.scan_id.append(self._counter)
