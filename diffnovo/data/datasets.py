...
class SpectrumDataset(Dataset):
    ...
    def __getitem__(self, idx) -> Tuple[torch.Tensor, float, int, str]:
        ...
        spectrum_id: str
            ...
            original peak file.
        """
        mz_array, int_array, precursor_rt, precursor_mz, precursor_charge = self.index[idx]
        spectrum = self._process_peaks(
            mz_array, int_array, precursor_mz, precursor_charge
        )
        return spectrum, precursor_rt, precursor_mz, precursor_charge, str(idx)

    def kth(self,v, k):
        return np.partition(v, k)[k]
    
    def clipn(self, *kw, sigma=4):
        return np.clip(np.random.randn(*kw), -sigma, sigma) / sigma

    def _data_enhance(self, mzs, its):
        #its = normalize(its, self.hyper.mode)
        #for training 
        #do normalization
        its = np.sqrt(np.sqrt(its))

        if len(mzs) > 500:
            th = self.kth(its, int(abs(self.clipn(sigma=2)) * self.hparam_kth))
    ...
    def _process_peaks(
        ...
        torch.Tensor of shape (n_peaks, 2)
            A tensor of the spectrum with the m/z and intensity peak values.
        """
        #print('before', len(mz_array), mz_array[-1])
        #print('before intensity', int_array)
        #mz_array, int_array = self._data_enhance(mz_array, int_array)
        #print('after mz', mz_array)
        #print('after', len(mz_array), mz_array[-1])
...
class AnnotatedSpectrumDataset(SpectrumDataset):
    ...
    def __getitem__(self, idx: int) -> Tuple[torch.Tensor, float, int, str]:
        ...
        #print(spectrum)
        return spectrum, precursor_rt, precursor_mz, precursor_charge, peptide
