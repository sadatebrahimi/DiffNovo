...
def annotate_mgf_one_spectrum_multiple_precursor(spect_f, spectrum_location_dict, precursor_dict, model, all_scans = 'all'):
  ...
  out_f = spect_f.split('.mgf')[0]+'_'+model+ '_'+ all_scans +'.mgf'
  with open (spect_f, mode='r') as input_file_handle:
    with open(out_f, mode='w') as output_handle:
      for scan, precursor_list in precursor_dict.items():
        for precursor in precursor_list:
          #precursor [middel_scan_list, mz, z, rt_mean, seq]
          # [(['F1:2022', 'F1:2049', 'F1:2076', 'F1:2103', 'F1:2130'], 444.21304000000003, 2, 3.1655995999999997, 'SC(+57.02)HTAVGR')]
  ...
  with open (path+spec_f, mode='r') as input_file_handle:
    ...
    with open(path+'test_plasma_pepnet.mgf', mode='w') as output_handle:
      ...
      while line:
            ...
            # each precursor is a tuple of (spec_grouop_id, mz, charge, rtime, seq) like(583.28314, 2, 11.1451, 'AATGEC(+57.02)TATVGK')
            line_buffer = []
            assert "BEGIN IONS" in line, "Error: wrong input BEGIN IONS"
            line_buffer.append(line)
            line = input_file_handle.readline()
...
def extract_precursor_5_middile_info_by_spec_group_id(feature_f, spectrum_rtinseconds_dict):
  ...
  for _, row in feature_df.iterrows():
    ...
    #
    #   if scan in d:
    #     if d[scan][0][3] == seq and int(d[scan][0][0]) == int(mz):
    #       continue
    #   if len(d[scan]) > 1: print(scan, d[scan])
    #   scan_l.append(scan)
    #   scan_set.add(scan)
    #   d[scan].append((spec_group_id, mz, z, rt_mean, seq))
...
print("This script annotate mgf files for pepnet and transformer_DIA")
print("Please provide the model name, the mgf file, and feature file")
model_name = input("Enter the name of model feeded the annotated mgf, it's ethier pepnet or transdia:")
input_spectrum_file = input("Enter the spectrum file name (with .mgf extension):")
input_feature_file = input("Enter the feature file name (.csv entension):")
#one_scan_one_spectrum = input("Enter if one or multiple precursor for each spectrum for annotation (one or more)")

all_spectra = input("Enter if you need to annotate all spectra or not (enter 'all' or 'five_rt' or 'five_int' all (means annotate all spectra )"
                    " or 'five_rt'(annotates top five sacns with cloasest retention time to the mean_rt) or "
                    "'five_int' (means annotate with top five scans with highest intensity values for):")
#add_seq_to_mgf_dia(path, spec_file, feature_file)
#add_dup_seq_to_mgf_dia(path, spec_file, feature_file)
input_spectrum_handle, input_feature_handle = open_input(input_spectrum_file, input_feature_file)
spectrum_location_dict, spectrum_rtinseconds_dict = get_location(input_spectrum_file)
#all_precursor_info = extract_all_features_by_scan(input_feature_file, spectrum_rtinseconds_dict)
#annotate_mgf_one_spectrum_one_precursor(input_spectrum_file, spectrum_location_dict, all_precursor_info, model_name, True)

if all_spectra == 'all':
    all_precursor_info = extract_all_features_by_spec_group_id(input_feature_file, spectrum_rtinseconds_dict)
    #all_precursor_info = extract_all_features_by_scan(input_feature_file, spectrum_rtinseconds_dict) #extract_all_features_by_scan(input_feature_file, spectrum_rtinseconds_dict)
    annotate_mgf_one_spectrum_one_precursor(input_spectrum_file, spectrum_location_dict, all_precursor_info, model_name, True)

##For each precursor ion, find five spectra in the MGF file with highest intensity
elif all_spectra =='five_int': 
    precursor_info = extract_precursor_5_highest_intensity_info_by_spec_group_id(input_feature_file, input_spectrum_handle, spectrum_location_dict)
#For each precursor ion, find five spectra in the MGF file with the closest retention times 
#to the mean retention time of the spectra.
elif all_spectra =='five_rt':
    precursor_info = extract_precursor_5_middile_info_by_spec_group_id(input_feature_file, spectrum_rtinseconds_dict)
annotate_mgf_one_spectrum_multiple_precursor(input_spectrum_file, spectrum_location_dict, precursor_info, model_name, all_spectra)
